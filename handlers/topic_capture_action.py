...
from src.plugin_system.apis import llm_api, send_api, message_api
...
class TopicCaptureAction(BaseAction):
    ...
    async def execute(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            # 2. 调用LLM进行分析
            should_intervene, reasoning, reply_content = await self._analyze_context(recent_messages)
    ...
    async def _get_recent_messages(self, stream_id: str, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 注意：message_api.get_history 通常返回 Message 列表
            messages = await message_api.get_history(stream_id, limit=limit)
    ...
    async def _analyze_context(self, messages: List[Dict[str, Any]]) -> Tuple[bool, str, str]:
        """
        使用LLM分析上下文
...
"""
            ...
            response = await llm_api.generate_text(prompt, **model_config)
